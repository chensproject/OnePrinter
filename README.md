...
* 支持打印日志管理，包括详情、暂停、删除、清空、审核、拒绝等操作。
* 支持隔空打印的打印审核，开启后打印请求需审核后才发起打印。
...
* 模板打印模块，支持拖拽设计、多张纸、批量excel数据导入、批量打印、接口打印、导出pdf、导出图片、导出字段等；
...
* 图片打印自适应（部分打印机没有无边框模式时也可以无边框）；

**修复：**

* 隔空打印、设备管理的详情页返回列表时，不在当前页码的问题；
